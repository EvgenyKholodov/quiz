...
export default class Study extends React.Component {
	...
	render() {
		console.log('this.state.quiz.questions🎅🎅🎅🎅🎅', this.state.quiz.questions);
		...
		return (
			<div className = 'studyWrapper'>
				<div className = 'studyContainer'>
					<h1 className = 'quizTitle'>{Parser(this.props.quiz.title)}</h1>

					{ !this.state.complete &&
					<div>
						<h3 className = 'quizLength'>Question {this.state.index + 1} of {quiz.questions.length}</h3>
						<h1 className = 'questionTitle'>{Parser(currentQuestion.questionTitle)}</h1>
					</div> }
